...
export function NewRecipeClient({ workspaceSlug, canSeeFinancials }: { workspaceSlug: string; canSeeFinancials: boolean }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body = {
        ...
        // Financial fields: only sent when this role can actually set them.
        // The API independently enforces this (recipe.update_cost) and
        // silently drops these fields for Chef/Staff regardless -- this is
        // just not pretending the values would take effect.
        ...(canSeeFinancials ? {
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className={canSeeFinancials ? "grid grid-cols-1 lg:grid-cols-3 gap-6" : "grid grid-cols-1 gap-6"}>
                ...
                <table className="w-full text-sm">
                  <thead className="text-[10px] uppercase tracking-wider text-text-tertiary border-b border-bg-border bg-bg-inset sticky top-0">
                    <tr>
                      ...
                      {canSeeFinancials && <th className="text-right px-3 py-2 font-medium w-24">Line cost</th>}
                      <th className="w-8" />
                    </tr>
                  </thead>
        ...
        {/* ---- RIGHT COLUMN (sticky cost summary) -- omitted entirely for
             roles without financial visibility, not dashed out. ---- */}
        {canSeeFinancials && (
        ...
        {/* Save button + hint, shown standalone under the ingredients column
            when the cost-summary sidebar (which normally carries them) is
            hidden. */}
        {!canSeeFinancials && (
          <div className="flex items-center gap-3">
            <Button onClick={handleSubmit} disabled={!canSubmit} loading={submitting}>
            ...
            {filledLines.length < 1 && (
              <p className="text-xs text-text-tertiary">Add at least one ingredient to save.</p>
            )}
          </div>
        )}
      </div>
