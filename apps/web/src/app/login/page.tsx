...
function LoginPageInner() {
  ...
  // Guard against open-redirect via the `next` query param: only allow
  // in-app paths (must start with a single slash). Anything else falls
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      // First-login flow: admin-created users carry mustChangePassword
      // until they rotate the temporary password. Send them through
      // /account/change-password first, then back to `next`.
      if (res.data.user.mustChangePassword) {
        // The route literal is known to typedRoutes; the query string
        // suffix is allowed for any valid base route.
        router.push(
          `/account/change-password?next=${encodeURIComponent(safeNext)}` as Route,
        );
      } else {
        // `safeNext` is a runtime string, so typedRoutes can't infer
        // it. Cast through Route — the value has already been
        // constrained to an in-app path above.
