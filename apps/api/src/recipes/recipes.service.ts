...
export class RecipesService {
  ...
  /**
   * Pure compute — no DB writes. Used by recost() AND by the breakdown
  ...
  computeBreakdown(recipe: any): RecipeCostBreakdown {
    ...
    for (const link of recipe.ingredients) {
      ...
      try {
        ...
        // 5% trim loss → we need MORE raw to get the cooked quantity,
        ...
        // Cost: canonical-qty × microcents-per-canonical. Round to int.
