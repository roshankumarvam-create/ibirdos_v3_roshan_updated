...
export class InsightsService {
  async list(ctx: TenantContext, opts: { status?: string; kind?: string; severity?: string; limit?: number }) {
  ...
  async acknowledge(ctx: TenantContext, id: string) {
    const i = await prisma.insight.findFirst({ where: { id, workspaceId: ctx.workspaceId } });
    if (!i) throw new NotFoundException({ code: "not_found", message: "Insight not found" });
  ...
  async markActioned(ctx: TenantContext, id: string) {
    const i = await prisma.insight.findFirst({ where: { id, workspaceId: ctx.workspaceId } });
    if (!i) throw new NotFoundException({ code: "not_found", message: "Insight not found" });
