...
import { prisma, Prisma, writeAudit, type TenantContext } from "@ibirdos/db";
...
export class CustomerOrderingService {
  ...
  async buildQuote(input: BuildQuoteInput) {
    ...
    const order = await prisma.customerOrder.create({
      data: {
        ...
        addonsJson: input.addons ?? Prisma.DbNull,
   ...
   */
  async confirmPayment(orderId: string, params: { stripePaymentIntentId?: string }): Promise<any> {
