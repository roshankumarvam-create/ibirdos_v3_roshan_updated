...
export class NotificationsService {
  ...
  /** Publish a notification â€” persisted AND pushed to socket subscribers */
  async publish(params: {
    ...
    // Push via Redis pub/sub â€” the Phase 14 socket gateway bridges to clients
  ...
  async list(ctx: TenantContext, opts: { unreadOnly?: boolean; limit?: number }): Promise<any> {
    const where: any = {
  ...
  async markRead(ctx: TenantContext, id: string): Promise<any> {
