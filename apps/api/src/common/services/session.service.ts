...
export class SessionService {
  ...
  /**
   * Resolve the Session row id from the request cookie. Used by flows
   * that want to revoke every OTHER session for a user (e.g. password
   * change) while keeping the current request authenticated.
   */
  async currentSessionId(req: Request): Promise<string | null> {
    const token = req.cookies?.[env.AUTH_COOKIE_NAME];
