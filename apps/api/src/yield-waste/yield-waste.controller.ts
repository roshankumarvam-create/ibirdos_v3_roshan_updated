...
export class YieldWasteController {
  ...
  recordYield(@CurrentCtx() ctx: TenantContext, @Body(new ZodValidationPipe(YieldSchema)) body: any): Promise<any> {
    return this.svc.recordYield(ctx, body).then(ok);
  }

  @Get("yield") @RequirePermission("yield.read")
  listYield(@CurrentCtx() ctx: TenantContext, @Query() q: any): Promise<any> {
  ...
  recordWaste(@CurrentCtx() ctx: TenantContext, @Body(new ZodValidationPipe(WasteSchema)) body: any): Promise<any> {
    return this.svc.recordWaste(ctx, body).then(ok);
  }

  @Get("waste") @RequirePermission("waste.read")
  listWaste(@CurrentCtx() ctx: TenantContext, @Query() q: any): Promise<any> {
