...
export class EventsService {
  ...
  async list(ctx: TenantContext, opts: { status?: string; upcoming?: boolean; cursor?: string; limit?: number }) {
  ...
  async addMenuItem(ctx: TenantContext, eventId: string, input: { recipeId: string; portions: number; perItemMultiplier?: number; notes?: string }) {
  ...
  async assignStaff(ctx: TenantContext, eventId: string, input: { userId?: string; role: string; hours: number; hourlyRateCents: number; notes?: string }) {
    const event = await prisma.event.findFirst({ where: { id: eventId, workspaceId: ctx.workspaceId, deletedAt: null } });
    if (!event) throw new NotFoundException({ code: "not_found", message: "Event not found" });
  ...
  // -----------------------------------------------------------------
  // Kitchen packet generation — the spec's flagship event capability
  // -----------------------------------------------------------------

  /**
   * Walks the event menu, aggregates ingredient demand across all
   * recipes (scaled by portions × multipliers), and persists a
   * KitchenPacket snapshot with:
   *   ingredients[] — totals per ingredient with per-recipe breakdown
   *   tasks[] — per-recipe production tasks (portions, prep/cook times)
   */
  async generateKitchenPacket(ctx: TenantContext, eventId: string) {
    ...
    // ingredientId → { name, canonicalUnit, totalCanonical, breakdown: [{recipeId,recipeName,qty}] }
