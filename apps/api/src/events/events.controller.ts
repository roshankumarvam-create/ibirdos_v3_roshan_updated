...
export class EventsController {
  ...
  create(@CurrentCtx() ctx: TenantContext, @Body(new ZodValidationPipe(CreateEventSchema)) body: any) {
  ...
  addMenu(@CurrentCtx() ctx: TenantContext, @Param("id") id: string,
          @Body(new ZodValidationPipe(AddMenuSchema)) body: any) {
  ...
  assignStaff(@CurrentCtx() ctx: TenantContext, @Param("id") id: string,
              @Body(new ZodValidationPipe(AssignStaffSchema)) body: any) {
    return this.svc.assignStaff(ctx, id, body).then(ok);
  }

  @Post(":id/kitchen-packet/generate") @RequirePermission("event.update")
  generatePacket(@CurrentCtx() ctx: TenantContext, @Param("id") id: string) {
